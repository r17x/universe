---
name: verify-nix
description: Verify Nix
updated: "2026-05-10"
---

Fast Nix verification commands ordered by speed, from sub-second eval checks to full flake validation.
...
Verification results as pass/fail per command:
```bash
nix flake check --no-build  # ✓ or ✗ with error
nix eval ...                 # ✓ or ✗ with error
nix fmt -- --check .         # ✓ or ✗ with diff
```
