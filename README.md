...
graph LR
  ...
  registry_users["registry-users"]:::registry_users_c
  ...
  user___self_user__0{{"user/<self:user>:0"}}:::user___self_user__0_c
  ...
  classDef registry_users_c fill:#F17E84,stroke:#F17E84,color:#2B2D3A,stroke-width:2px,stroke-dasharray: 8 4
  ...
  classDef user___self_user__0_c fill:#F5B083,stroke:#F5B083,color:#2B2D3A,stroke-dasharray: 3 3,stroke-width:1px
```
...
```
Registry (data)  →  Schema (typed options)  →  Aspects (wiring)
   you edit           from module systems        never touch
```

### User Registry

All personal preferences live in `r17.nix` (`den.users.registry.r17`). Schema types are defined in `nix/den/schema/user.nix`. Aspects read from `user.*` — changing a preference means editing `r17.nix`, never touching aspect files.
