...
const toPointer = (n: number) => {
  if (isPointer(n)) return n;
  throw new FFFError({ query: "<ffi>", message: "Invalid null pointer" });
};

export const assertPtr = (p: Pointer | null, context: string) => {
  if (!p) throw new FFFError({ query: context, message: `Null pointer in ${context}` });
  return p;
};

const readFfiString = (rawPtr: Pointer | null) => {
  if (!rawPtr) return "";
  return new CString(rawPtr).toString();
};

const readCString = (rawPtr: number | null) => {
  if (!rawPtr) return "";
  return new CString(toPointer(rawPtr)).toString();
};

const readFfiBool = (val: boolean) => val;

const readResultSuccess = (resultPtr: Pointer) => read.u8(resultPtr, RESULT_SUCCESS_OFFSET) !== 0;

const readResultError = (resultPtr: Pointer) => {
  const errorRaw = read.ptr(resultPtr, RESULT_ERROR_OFFSET);
  return errorRaw ? new CString(toPointer(errorRaw)).toString() : null;
};

const readResultHandle = (resultPtr: Pointer) =>
  toPointer(read.ptr(resultPtr, RESULT_HANDLE_OFFSET));

export const readResultIntValue = (resultPtr: Pointer) =>
  read.i64(resultPtr, RESULT_INT_VALUE_OFFSET);

export const unwrapResult = (lib: FffLib, resultPtr: Pointer | null, query: string) => {
...
export const unwrapResultVoid = (lib: FffLib, resultPtr: Pointer | null, query: string) => {
  if (!resultPtr) throw new FFFError({ query, message: "FFI returned null result" });
  if (!readResultSuccess(resultPtr)) {
    const error = readResultError(resultPtr) ?? "Unknown FFI error";
    lib.symbols.fff_free_result(resultPtr);
...
export const unwrapResultBool = (lib: FffLib, resultPtr: Pointer | null, query: string) => {
  if (!resultPtr) throw new FFFError({ query, message: "FFI returned null result" });
  if (!readResultSuccess(resultPtr)) {
    const error = readResultError(resultPtr) ?? "Unknown FFI error";
    lib.symbols.fff_free_result(resultPtr);
...
export const unwrapResultInt = (lib: FffLib, resultPtr: Pointer | null, query: string) => {
  if (!resultPtr) throw new FFFError({ query, message: "FFI returned null result" });
  if (!readResultSuccess(resultPtr)) {
    const error = readResultError(resultPtr) ?? "Unknown FFI error";
    lib.symbols.fff_free_result(resultPtr);
...
export const unwrapResultString = (lib: FffLib, resultPtr: Pointer | null, query: string) => {
  if (!resultPtr) throw new FFFError({ query, message: "FFI returned null result" });
  if (!readResultSuccess(resultPtr)) {
    const error = readResultError(resultPtr) ?? "Unknown FFI error";
    lib.symbols.fff_free_result(resultPtr);
...
export const validateLibrary = (lib: FffLib, handle: Pointer) => {
  const resultPtr = lib.symbols.fff_health_check(handle, null);
  if (!resultPtr)
    throw new FFFError({
      query: "<validate>",
      message: "Library health check returned null — incompatible version?",
    });
  if (!readResultSuccess(resultPtr)) {
...
export const extractScore = (scorePtr: Pointer) => {
  if (!scorePtr)
    return {
      total: 0,
      baseScore: 0,
...
export const extractLocation = (basePtr: Pointer, tagOffset: number) => {
...
export const extractFileItem = (lib: FffLib, itemPtr: Pointer) => ({
  ...
  modificationFrecencyScore: Number(
    lib.symbols.fff_file_item_get_modification_frecency_score(itemPtr),
  ),
...
export const extractMatchRange = (rangePtr: Pointer) => ({
  start: read.u32(rangePtr, MR_START),
  end: read.u32(rangePtr, MR_END),
});

export const extractGrepMatch = (lib: FffLib, matchPtr: Pointer) => {
  const matchRangesCount = lib.symbols.fff_grep_match_get_match_ranges_count(matchPtr);
  const matchRanges = (
    matchRangesCount > 0
      ? Arr.makeBy(matchRangesCount, (i) => lib.symbols.fff_grep_match_get_match_range(matchPtr, i))
      : []
  ...
  const contextBefore =
    contextBeforeCount > 0
      ? Arr.makeBy(contextBeforeCount, (i) =>
  ...
  const contextAfter =
    contextAfterCount > 0
      ? Arr.makeBy(contextAfterCount, (i) =>
  ...
  return {
    ...
    modificationFrecencyScore: Number(
      lib.symbols.fff_grep_match_get_modification_frecency_score(matchPtr),
    ),
...
export const extractSearchResult = (lib: FffLib, searchResultPtr: Pointer) => {
  ...
  const entries = [
    ...
    (
      e,
    ): e is { itemPtr: NonNullable<typeof e.itemPtr>; scorePtr: NonNullable<typeof e.scorePtr> } =>
...
export const extractGrepResult = (lib: FffLib, grepResultPtr: Pointer) => {
  const count = lib.symbols.fff_grep_result_get_count(grepResultPtr);
  const items = (
    count > 0
      ? Arr.makeBy(count, (i) => lib.symbols.fff_grep_result_get_match(grepResultPtr, i))
      : []
...
export const extractDirItem = (dirItemPtr: Pointer) => {
...
export const extractDirSearchResult = (lib: FffLib, dsrPtr: Pointer) => {
...
export const extractMixedSearchResult = (lib: FffLib, msrPtr: Pointer) => {
...
export const extractScanProgress = (spPtr: Pointer) => {
  if (!spPtr)
    return {
      scannedFilesCount: 0,
      isScanning: false,
