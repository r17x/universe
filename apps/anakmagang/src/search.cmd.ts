...
const grepFlags = {
  mode: Flag.choice("mode", ["plain", "regex", "fuzzy"] as const).pipe(
    Flag.withAlias("m"),
    Flag.optional,
  ),
...
const findCommand = Command.make(
  ...
  ({ query, ...flags }) =>
    Effect.gen(function* () {
      ...
      yield* Effect.forEach(Arr.zip(result.items, result.scores), ([item, score]) =>
        output.emit(
          Record({
            fields: [
...
const grepCommand = Command.make(
  ...
  ({ query, ...flags }) =>
    Effect.gen(function* () {
      ...
      yield* Effect.forEach(result.items, (item) =>
        output.emit(
          Record({
            fields: [
              ["location", `${item.path}:${item.lineNumber}`],
              ["content", item.lineContent],
            ],
          }),
        ),
      );
...
const multiGrepCommand = Command.make(
  "multi-grep",
  {
    patterns: Argument.string("patterns").pipe(Argument.withSchema(Schema.NonEmptyString)),
    ...grepFlagsWithoutMode,
  },
  ({ patterns, ...flags }) =>
    Effect.gen(function* () {
      const output = yield* Output;
...
const searchParent = Command.make("search", {}, () =>
  Effect.gen(function* () {
    ...
    yield* output.emit(
      Line({
        text: "Usage: anakmagang search <find|grep|multi-grep|find-dirs|find-mixed> <query>",
      }),
    );
  }),
);

export const searchCommand = Command.withSubcommands(searchParent, [
  findCommand,
  grepCommand,
  multiGrepCommand,
  findDirsCommand,
