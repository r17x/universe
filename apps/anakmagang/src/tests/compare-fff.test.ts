...
const LIBFFF_DIR = process.env.LIBFFF_PATH;
if (!LIBFFF_DIR)
  throw new Error("LIBFFF_PATH not set — run tests inside `nix develop .#anakmagang`");
const LIB_PATH = `${LIBFFF_DIR}/libfff_c.dylib`;
...
const doSearch = (query: string, pageSize = 100): SearchResult => {
  const resultPtr = lib.symbols.fff_search(handle, buf(query), null, 0, 0, pageSize, 0, 0);
...
const doDirSearch = (query: string, pageSize = 100): DirSearchResult => {
  const resultPtr = lib.symbols.fff_search_directories(handle, buf(query), null, 0, 0, pageSize);
...
const doMixedSearch = (query: string, pageSize = 100): MixedSearchResult => {
  const resultPtr = lib.symbols.fff_search_mixed(handle, buf(query), null, 0, 0, pageSize, 0, 0);
...
const doHealthCheck = (testPath?: string): unknown => {
  ...
  return jsonStr ? Schema.decodeUnknownSync(Schema.fromJsonString(Schema.Unknown))(jsonStr) : null;
};
...
describe("FFF binding - grep", () => {
  ...
  test("all grep results contain 'import' in lineContent", () => {
    const result = doGrep("import");
    const MAX_REASONABLE_LINE = 500;
    for (const item of result.items) {
      if (item.lineContent.length >= MAX_REASONABLE_LINE) continue;
